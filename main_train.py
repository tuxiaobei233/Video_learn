...
train_batch_size = 16
num_epochs = 4
model_save_path = 'output/training_stsbenchmark-' + \
    ...
    for row in reader:
        ...
        if row['split'] == 'dev':
            ...
            train_samples.append(InputExample(
                texts=[row['sentence1'], row['sentence2']], label=score))
            train_samples.append(InputExample(
                texts=[row['sentence2'], row['sentence1']], label=score))
...
for features, labels in tqdm(train_dataloader, desc="Iteration", smoothing=0.0):
    print(features)
    print(labels)
    break
